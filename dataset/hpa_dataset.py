    ...
    def get_stratified_samplers(self, fold=-1):
        ...
        for fold, (train_index, test_index) in enumerate(mskf.split(X, y)):
            ...
            if self.writer:
                ...
                tr = ax.bar(np.arange(len(y_t_dict)) -0.25, y_t_dict, width=0.4, color='tab:red', log=True)
                ev = ax.bar(np.arange(len(y_e_dict)) +0.25, y_e_dict, width=0.4, color='tab:blue', log=True)
                ax.legend((tr[0], ev[0]), ('trian', 'eval'))
                ax.set_ylabel('exp', color='tab:blue')
                for i, v in enumerate(y_t_dict): ax.text(i - 0.25, v + 3, str(v), color='red', fontweight='bold')
                for i, v in enumerate(y_e_dict): ax.text(i + 0.25, v + 3, str(v), color='blue', fontweight='bold')
