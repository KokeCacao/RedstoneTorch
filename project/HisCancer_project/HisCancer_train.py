    ...
    def __init__(self, writer):
        ...
        for fold in range(config.MODEL_FOLD):
            if fold not in config.MODEL_TRAIN_FOLD:
                ...
                self.lr_schedulers.append(PlateauCyclicRestart(optimizer, eval_mode='max', factor=0.2, patience=1, verbose=False, threshold=1e-4, threshold_mode='abs', cooldown=0, eps=1e-8, base_lr=0.001, max_lr=0.01, step_size=2705, mode='plateau_cyclic', gamma=1., scale_mode='cycle', last_batch_iteration=-1, reduce_restart=3))
    ...
    def step_fold(self, fold, net, optimizer, lr_scheduler, batch_size):
        ...
        for train_index in tqdm(range(ratio)):
            ...
            for batch_index, (ids, image, labels_0, image_for_display) in enumerate(pbar):
                ...
                lr_scheduler.step(0, config.epoch, config.global_steps[fold])
                """TRAIN NET"""
