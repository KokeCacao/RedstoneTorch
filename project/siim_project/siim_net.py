...
class model34_DeepSupervion_GroupNorm_OC(nn.Module):
    ...
    def __init__(self, num_classes=1, mask_class = 1):
        ...
        # self.logits_final = nn.Sequential(nn.Conv2d(64*5+64, 64, kernel_size=3, padding=1),
        #                                  nn.LeakyReLU(inplace=True),
        #                                  nn.Conv2d(64, 1, kernel_size=1, padding=0))

        self.logits_final = nn.Sequential(ocnet.ASP_OC_Module(64*2+64, 64),
                                          nn.LeakyReLU(inplace=True),
                                          nn.Conv2d(64, 1, kernel_size=1, padding=0))
    ...
    def forward(self, x, flip):
        ...
        center_64 = self.center_conv1x1(torch.cat((self.center_global_pool(conv5), flip.view(-1, 1, 1, 1)), 1))
