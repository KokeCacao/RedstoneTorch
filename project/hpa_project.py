...
class HPAProject:
    def __init__(self, writer):
        ...
        for fold in range(config.MODEL_FOLD):
            ...
            self.optimizers = self.optimizers.append(torch.optim.Adam(params=net.parameters(), lr=config.MODEL_LEARNING_RATE, betas=(0.9, 0.999), eps=1e-08, weight_decay=config.MODEL_WEIGHT_DEFAY))  # all parameter learnable
            self.nets = self.nets.append(cuda(net))
