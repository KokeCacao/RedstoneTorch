...
class Bottleneck_GroupNorm(nn.Module):
    expansion = 4
...
class ResNet_GroupNorm(nn.Module):

    def __init__(self, block, layers, num_classes=1000):
        self.inplanes = 64
        super(ResNet_GroupNorm, self).__init__()
        """MODIFY TO HAVE 1 DIMENSION INPUT"""
        self.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
...
def resnet18(pretrained=False, **kwargs):
    ...
    if pretrained:
        model = modified_initialize_pretrained_model(model, model_urls['resnet18'])
    return model
...
def resnet34_GroupNorm(pretrained=False, **kwargs):
    """Constructs a ResNet-34 model.
...
def resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        model = modified_initialize_pretrained_model(model, model_urls['resnet50'])
    return model
...
def resnet101(pretrained=False, **kwargs):
    ...
    if pretrained:
        model = modified_initialize_pretrained_model(model, model_urls['resnet101'])
    return model
