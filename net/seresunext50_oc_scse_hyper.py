...
__all__ = ['SEResNet50', 'SEResNet101', 'SEResNeXt50', 'SEResNeXt101', 'SeResUNeXtscSEOCHyper50']
...
class SeResUNeXtscSEOCHyper50(nn.Module):
    def __init__(self, num_classes=1, dilation=False):
        super(SeResUNeXtscSEOCHyper50, self).__init__()
        self.dilation = dilation
        self.encoder = se_resnext50_32x4d(num_classes, input_channel=num_classes)
        ...
        self.conv1x1_5 = nn.Conv2d(128, 4, 1)
        self.conv1x1_4 = nn.Conv2d(128, 4, 1)
        self.conv1x1_3 = nn.Conv2d(64, 4, 1)
    ...
    def forward(self, x, flip):
        ...
        print(F.upsample(self.conv1x1_2(d2), scale_factor=1, mode='bilinear', align_corners=False).shape)
        print(F.upsample(self.conv1x1_3(d3), scale_factor=2, mode='bilinear', align_corners=False).shape)
        print(F.upsample(self.conv1x1_4(d4), scale_factor=4, mode='bilinear', align_corners=False).shape)
        print(F.upsample(self.conv1x1_5(d5), scale_factor=8, mode='bilinear', align_corners=False).shape)
...
class SENet(nn.Module):
    def __init__(self, block, layers, groups, reduction, dropout_p=0.2,
                 ...
                 downsample_padding=1, num_classes=None, input_channel=3):
        """
        Parameters
        ----------
...
def senet154(num_classes=1000, pretrained='imagenet', input_channel=3):
    model = SENet(SEBottleneck, [3, 8, 36, 3], groups=64, reduction=16,
                  dropout_p=0.2, num_classes=num_classes, input_channel=input_channel)
...
def se_resnet101(num_classes=1000, pretrained='imagenet', input_channel=3):
...
def se_resnet152(num_classes=1000, pretrained='imagenet', input_channel=3):
...
def se_resnext50_32x4d(num_classes=1000, input_channel=3):
...
def se_resnext101_32x4d(num_classes=1000, pretrained='imagenet', input_channel=3):
